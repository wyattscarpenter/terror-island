#For testing purposes, you may pass this script an argument for which step to start at. 1 to run every step, 2 to skip the first step, 3 to skip the first two steps. 4 or above to exist immediately (there are only 3 steps).

# Step 1: This line does a basic pass to get all of the webpages we need. If need be, you can also just run this line and get a pretty good result (there is no way to do that using this script; I'm just saying you can run it on your own):
if [ "${1:-0}" -le 1 ] ; then
...
# Step 2: Getting the prereq from the "other domain" terrorisland.net was very easy, but for the rest of the internet we need to do it sort of "manually". We do this by downloading all the files again. This is bad, because wget should just have been able to do this all in one sweep. But it's only O(2n) bad, so, you know...
if [ "${1:-0}" -le 2 ] ; then
